...
$('body').on('click', '#newBeer', function () {
    $.ajax({
        ...
        data.forEach(function (element) {
            ...
            $('.modal').empty();
            $('.modal').append('<div><h1>INGREDIENTS</h1><p class="close">x</p></div><ul><li><span>Malt:</span></li><li>' + element.ingredients.malt[0].name + '</li></ul><ul><li><span>Hops:</span></li><li>' + element.ingredients.hops[0].name + '</li></ul><ul><li><span>Yeast:</span></li><li>' + element.ingredients.yeast + '</li></ul>');
        });
